...
export default function Header({
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm">
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 h-18 flex items-center justify-between gap-1 sm:gap-4">
        {/* Logo */}
        <div 
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5">
            {logoConfig.imageUrl ? (
              <img 
                ...
                className="h-10 max-w-[160px] object-contain transition-transform group-hover:scale-102 shrink-0"
                referrerPolicy="no-referrer"
              />
            ) : (
              <div className="flex items-center gap-1.5">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-blue-600 flex items-center justify-center shadow-md shadow-blue-500/20 group-hover:scale-105 transition-transform shrink-0">
                  <span className="font-black text-white text-sm sm:text-base tracking-tighter">{logoConfig.text}</span>
                </div>
                <div className="flex flex-col">
                  <span className="font-extrabold text-base sm:text-xl tracking-tighter text-blue-900 leading-none whitespace-nowrap">
                    {logoConfig.text}<span className="text-blue-600">.{logoConfig.subtitle.split('.').slice(1).join('.') || 'com'}</span>
                  </span>
                  <span className="text-[8px] sm:text-[9px] text-slate-400 font-bold tracking-wider mt-0.5 uppercase">EV Charging</span>
                </div>
              </div>
            )}

            {logoConfig.showCompanyName && logoConfig.companyNameText && (
              <div className="hidden sm:flex pl-3 border-l border-slate-200 h-6 items-center shrink-0">
        ...
        <div className="flex items-center gap-1 sm:gap-3 shrink-0">
          ...
          {user ? (
            <div className="flex items-center gap-0.5 sm:gap-2.5 shrink-0">
              ...
              <button
                ...
                className="hidden md:flex items-center gap-1.5 py-1.5 px-3 bg-slate-50 hover:bg-slate-100 rounded-full border border-slate-200 text-xs font-semibold text-slate-700 transition-all cursor-pointer whitespace-nowrap shrink-0"
              >
              ...
              <button
                ...
                className="flex items-center gap-1 py-1.5 sm:py-2 px-1.5 sm:px-3 text-slate-600 hover:text-slate-950 hover:bg-slate-50 rounded-xl text-xs font-bold transition-all cursor-pointer whitespace-nowrap shrink-0"
              >
          ...
          <button
            onClick={onOpenQuote}
            id="btn-header-cta"
            className="relative overflow-hidden group py-1.5 sm:py-2.5 px-2 sm:px-5 bg-blue-600 hover:bg-blue-700 text-white rounded-full text-[10px] sm:text-sm font-bold shadow-lg shadow-blue-200 transition-all flex items-center gap-1 cursor-pointer shrink-0 whitespace-nowrap"
          >
            ...
            <span className="text-[9px] sm:text-[10px] bg-amber-400 text-amber-950 font-bold px-1 sm:px-1.5 py-0.5 rounded animate-pulse shrink-0">💡</span>
          </button>
        </div>
      </div>
    </header>
