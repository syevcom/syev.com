    ...
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between">
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            {logoConfig.showCompanyName && logoConfig.companyNameText && (
              <div className="hidden sm:flex pl-3 border-l border-slate-200 h-6 items-center">
        ...
        <div className="flex items-center gap-1.5 sm:gap-4">
          ...
          <div className="flex items-center gap-1 bg-slate-100 rounded-full p-1 border border-slate-200 shrink-0">
            <button
              onClick={onToggleEditMode}
              id="btn-header-edit-toggle"
              className={`flex items-center gap-1 py-1 px-2 sm:px-2.5 rounded-full text-[10px] font-black tracking-tight transition-all cursor-pointer ${
                isEditMode
            ...
            >
              ...
              <span className="sm:hidden">{isEditMode ? '수정 On' : '관리자'}</span>
            </button>
            {isEditMode && onOpenCms && (
              <button
                onClick={onOpenCms}
                id="btn-header-cms-open"
                className="flex items-center gap-1 py-1 px-2 sm:px-2.5 bg-slate-900 text-white rounded-full text-[10px] font-black cursor-pointer hover:bg-slate-800 transition-all shadow-sm"
              >
                <span className="hidden sm:inline">✏️ CMS 대시보드</span>
                <span className="sm:hidden">✏️ CMS</span>
              </button>
            )}
          </div>
          ...
          {user ? (
            <div className="flex items-center gap-1 sm:gap-2.5">
              ...
              <button
                ...
                className="flex items-center gap-1 py-1.5 sm:py-2 px-2 sm:px-3 text-slate-600 hover:text-slate-950 hover:bg-slate-50 rounded-xl text-xs font-bold transition-all cursor-pointer"
              >
          ...
          <button
            onClick={onOpenQuote}
            id="btn-header-cta"
            className="relative overflow-hidden group py-2 sm:py-2.5 px-2.5 sm:px-5 bg-blue-600 hover:bg-blue-700 text-white rounded-full text-[10px] sm:text-sm font-bold shadow-lg shadow-blue-200 transition-all flex items-center gap-1 cursor-pointer shrink-0"
          >
            ...
            <span className="sm:hidden">상담/예약</span>
